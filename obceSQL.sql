...
-- 6
select rok, sum(muzi + zeny) as popu from populacia
group by rok
order by rok desc;
-- 7
select o.nazov, p.zeny + p.muzi as popu from populacia p
inner join obec o on o.id = p.id_obec
...
limit 10;
-- 10
select k.nazov, count(o.nazov) as okres, count(o2.nazov) as obec, sum(p.zeny + p.muzi) from kraj k
